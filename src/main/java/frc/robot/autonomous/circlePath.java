...
public class circlePath extends genericAutonomous{
    ...
    PIDController xPIDControl = new PIDController(.1, 0, 0);
    PIDController yPIDControl = new PIDController(.1, 0, 0);
    @Override
    ...
    public void autonomousPeriodic(GenericRobot robot) {
        ...
        switch(autonomousStep){
            ...
            case 1:
                ...
                double s_0 = getS(t-increment);
                double s_1 = getS(t+increment);
                xspd = (velocityFunctionX(s_0) + velocityFunctionX(s_1))/2
                        + xPIDControl.calculate(-positionFunctionX(s_0) + currPose.getX());
                yspd = (velocityFunctionY(s_0) + velocityFunctionY(s_1))/2
                        + yPIDControl.calculate(positionFunctionY(s_0) - currPose.getY());
                turnspd = (velocityFunctionTheta(s_0) + velocityFunctionTheta(s_1))/2;
