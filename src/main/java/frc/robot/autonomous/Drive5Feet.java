...
public class Drive5Feet extends genericAutonomous {
    ...
    public void trajectoryCalc(GenericRobot robot, Pose2d origin, Pose2d dest) {
        ...
        xspd = (dx != dest.getX()) ? 5 : 0;
        yspd = (dy != dest.getY()) ? 5 : 0;

//        rspd = (dr != dest.getRotation()) ? 0 : 0;
        ChassisSpeeds targetChassisSpeeds = new ChassisSpeeds(xspd, yspd, 0);
    ...
    public Pose2d diff(GenericRobot robot, Pose2d one, Pose2d two) {
             return new Pose2d(two.getX()-one.getX(), two.getY()-one.getY(), Rotation2d.fromDegrees(Math.atan((two.getY()-one.getY())/(two.getX()-one.getX()))));
    }
